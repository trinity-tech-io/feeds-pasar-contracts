...
interface IPasarDataAndEvents {
    ...
    /**
     * @dev Seller fee info data structure
    ...
    /**
     * @dev Buyer fee info data structure
     ...
     * @param buyerAddr The address of the buyer
     * @param quoteToken The address of the token used as payment for the partially filled order
     ...
     * The `_tokenId` argument MUST be the token type placed on sale.
     * The `_amount` argument MUST be the amount of token placed on sale.
     * The `_quoteToken` argument MUST be the address of the token accepted as payment for the splittable order.
     ...
     * The `_royaltyOwner` argument MUST be the address of the royalty owner of the token sold in the order.
     * The `_quoteToken` argument MUST be the address of the token used as payment for the (partially) fulfilled order.
...
interface IPasarOrder {
     ...
     * @param _quoteToken The address of the token accepted as payment for the auction.
     ...
     */
    function buyOrder(uint256 _orderId, string calldata _didUri) external;
     ...
     * @param _value The price value of the bid
     * @param _didUri DID URI of the buyer
     */
    function bidForOrder(
        uint256 _orderId,
        uint256 _value,
        string calldata _didUri
    ) external;
...
interface IPasarUpgraded is IPasarDataAndEvents {
     ...
     * @param _tokenId The token type placed on sale
     * @param _amount The amount of token placed on sale
     * @param _quoteToken The address of the token accepted as payment for the splittable order.
    ...
    /**
     * @notice Get seller fee information on a quote token
     * @param _seller The address of the seller to query fee information with
     ...
     */
    function getBuyerFeeInfo(address _buyer, address _quoteToken) external view returns (BuyerFeeInfo memory);
}
...
abstract contract FeedsNFTPasarStorage is IPasarDataAndEvents, BaseUtils {
    ...
    mapping(address => mapping(address => SellerFeeInfo)) internal addrToSellerFees;
    mapping(address => BuyerInfo) internal addrToBuyer;
    mapping(address => mapping(address => BuyerFeeInfo)) internal addrToBuyerFees;
    ...
    FeedsNFTPasarStorage
{
    ...
    function createOrderForSale(
        ...
        callLibraryLogicContract(
            abi.encodeWithSignature("_createSalePre(uint256,uint256,uint256)", _tokenId, _amount, _price)
        );
        ...
        emit OrderForSale(msg.sender, newOrder.orderId, _tokenId, _amount, _quoteToken, _price);
    ...
    function createOrderForAuction(
        ...
        callLibraryLogicContract(
            abi.encodeWithSignature(
                "_createAuctionPre(uint256,uint256,uint256,uint256)",
                _tokenId,
                _amount,
                _minPrice,
                _endTime
        ...
        emit OrderForAuction(msg.sender, newOrder.orderId, _tokenId, _amount, _quoteToken, _minPrice, _endTime);
     ...
     * @param _didUri DID URI of the buyer
     */
    function buyOrder(uint256 _orderId, string calldata _didUri) external override inited reentrancyGuard {
        callLibraryLogicContract(abi.encodeWithSignature("_buyOrderPre(uint256)", _orderId));

        callLibraryLogicContract(abi.encodeWithSignature("_newOrderBuyer(address,uint256)", msg.sender, _orderId));
        callLibraryLogicContract(
            abi.encodeWithSignature(
                "_fillOrder(address,uint256,uint256)",
                msg.sender,
                _orderId,
    ...
    function bidForOrder(
        ...
        require(_value >= orders[_orderId].price && _value > orders[_orderId].lastBid, "Invalid bid");
        require(
            IERC20(orders[_orderId].quoteToken).allowance(msg.sender, address(this)) >= _value,
            "Not enough quote token approved for the bid"
        );
        require(
            IERC20(orders[_orderId].quoteToken).transferFrom(msg.sender, address(this), _value),
            "Quote token transfer failed for the bid"
        );

        callLibraryLogicContract(abi.encodeWithSignature("_newOrderBuyer(address,uint256)", msg.sender, _orderId));
        if (orders[_orderId].lastBidder != address(0x0)) {
            bool success = IERC20(orders[_orderId].quoteToken).transfer(
                orders[_orderId].lastBidder,
                orders[_orderId].lastBid
            );
            require(success, "Something is wrong in auction");
        }
        ...
        emit OrderBid(orders[_orderId].sellerAddr, msg.sender, _orderId, _value);
    ...
    function createSplittableOrder(
        ...
        callLibraryLogicContract(
            abi.encodeWithSignature("_createSplittablePre(uint256,uint256,uint256)", _tokenId, _amount, _price)
        );
        ...
        emit OrderSplittable(msg.sender, newOrder.orderId, _tokenId, _amount, _quoteToken, _price);
    ...
    function buySplittableOrder(
        ...
        require(
            IERC20(orders[_orderId].quoteToken).allowance(msg.sender, address(this)) >= buyPrice,
            "Not enough quote token approved for the order"
        );
        require(
            IERC20(orders[_orderId].quoteToken).transferFrom(msg.sender, address(this), buyPrice),
            "Quote token transfer failed for the order"
        );
...
contract FeedsNFTPasarLibrary is FeedsNFTPasarStorage {
    ...
    function _newOrderBuyer(address _buyer, uint256 _id) public inited {
    ...
    function _fillOrder(
        ...
        addrToSellerFees[seller][orders[_id].quoteToken].earned = _value
            .sub(orders[_id].royaltyFee)
            .sub(orderIdToExtraInfo[_id].platformFee)
            .add(addrToSellerFees[seller][orders[_id].quoteToken].earned);
        addrToSellerFees[seller][orders[_id].quoteToken].royalty = orders[_id].royaltyFee.add(
            addrToSellerFees[seller][orders[_id].quoteToken].royalty
        );
        addrToSellerFees[seller][orders[_id].quoteToken].platformFee = orderIdToExtraInfo[_id].platformFee.add(
            addrToSellerFees[seller][orders[_id].quoteToken].platformFee
        );

        buyerFilledOrders[_buyer].push(_id);
        addrToBuyer[_buyer].filledCount = buyerFilledOrders[_buyer].length;
        addrToBuyerFees[_buyer][orders[_id].quoteToken].paid = _value.add(
            addrToBuyerFees[_buyer][orders[_id].quoteToken].paid
        );
        addrToBuyerFees[_buyer][orders[_id].quoteToken].royalty = orders[_id].royaltyFee.add(
            addrToBuyerFees[_buyer][orders[_id].quoteToken].royalty
        );
        addrToBuyerFees[_buyer][orders[_id].quoteToken].platformFee = orderIdToExtraInfo[_id].platformFee.add(
            addrToBuyerFees[_buyer][orders[_id].quoteToken].platformFee
        );
        ...
        if (orders[_id].royaltyFee > 0) {
            require(
                IERC20(orders[_id].quoteToken).transfer(orders[_id].royaltyOwner, orders[_id].royaltyFee),
                "Royalty transfer failed"
            );
        }
        if (orderIdToExtraInfo[_id].platformFee > 0) {
            require(
                IERC20(orders[_id].quoteToken).transfer(
        ...
        require(
            IERC20(orders[_id].quoteToken).transfer(
                seller,
                orders[_id].filled.sub(orders[_id].royaltyFee).sub(orderIdToExtraInfo[_id].platformFee)
            ),
        ...
        emit OrderFilled(
            ...
            _buyer,
            _id,
            orders[_id].royaltyOwner,
            orders[_id].quoteToken,
            _value,
            orders[_id].royaltyFee
        );
    ...
    function _partialFill(
        ...
        newPartialFill.quoteToken = orders[_id].quoteToken;
        ...
        addrToSellerFees[seller][orders[_id].quoteToken].earned = _value
            .sub(newPartialFill.royaltyFee)
            .sub(newPartialFill.platformFee)
            .add(addrToSellerFees[seller][orders[_id].quoteToken].earned);
        addrToSellerFees[seller][orders[_id].quoteToken].royalty = newPartialFill.royaltyFee.add(
            addrToSellerFees[seller][orders[_id].quoteToken].royalty
        );
        addrToSellerFees[seller][orders[_id].quoteToken].platformFee = newPartialFill.platformFee.add(
            addrToSellerFees[seller][orders[_id].quoteToken].platformFee
        );
        ...
        addrToBuyerFees[_buyer][orders[_id].quoteToken].royalty = newPartialFill.royaltyFee.add(
            addrToBuyerFees[_buyer][orders[_id].quoteToken].royalty
        );
        addrToBuyerFees[_buyer][orders[_id].quoteToken].platformFee = newPartialFill.platformFee.add(
            addrToBuyerFees[_buyer][orders[_id].quoteToken].platformFee
        );
        ...
        if (newPartialFill.royaltyFee > 0) {
            require(
                IERC20(orders[_id].quoteToken).transfer(newPartialFill.royaltyOwner, newPartialFill.royaltyFee),
                "Royalty transfer failed"
            );
        }
        if (newPartialFill.platformFee > 0) {
            require(
                IERC20(orders[_id].quoteToken).transfer(newPartialFill.platformAddr, newPartialFill.platformFee),
                "Platform fee transfer failed"
            );
        }
        require(
            IERC20(orders[_id].quoteToken).transfer(
                seller,
                _value.sub(newPartialFill.royaltyFee).sub(newPartialFill.platformFee)
            ),
            "Payment transfer failed"
        );
        ...
        emit OrderPlatformFee(
            newPartialFill.platformAddr,
            newPartialFill.quoteToken,
            newPartialFill.platformFee,
            seller,
            _buyer,
            _id
        );
    ...
    function _settleAuction(uint256 _id) public inited {
    ...
    function _cancelOrder(uint256 _id) public inited {
    ...
    function _buyOrderPre(uint256 _orderId) public inited {
        require(orders[_orderId].orderType == 1 && orders[_orderId].orderState == 1, "Invalid order ID");
        require(
            IERC20(orders[_orderId].quoteToken).allowance(msg.sender, address(this)) >= orders[_orderId].price,
            "Not enough quote token approved for the order"
        );
        require(
            IERC20(orders[_orderId].quoteToken).transferFrom(msg.sender, address(this), orders[_orderId].price),
            "Quote token transfer failed for the order"
        );
    }

    function _createSplittablePre(
        uint256 _tokenId,
        uint256 _amount,
        uint256 _price
    ) public view inited {
